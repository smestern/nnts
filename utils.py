...
def compute_distro_mode(x, bin=5):
...
def compute_FI_curve(spike_times, time):
    FI_full = []
    isi=[]
    for r in spike_times:
        if len(r) > 1:
            FI_full.append(len(r))
            spike_times = np.diff(r)
            isi.append(compute_distro_mode(spike_times*1000))
        else:
            FI_full.append(0)
            isi.append(0) 
    return (np.hstack(FI_full) /time), np.hstack(isi) / 10 #in s
